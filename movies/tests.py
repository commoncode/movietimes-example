...
class PaginatedMovieTestCase(TestCase):
    ...
    def test_url_without_page_returns_most_recent(self):
        ...
        response_page1 = self.client.get('/movies/recent/?page=').json()
        self.assertListEqual(response_page1, movie_titles)

        response_page2 = self.client.get('/movies/recent/').json()
    ...
    def test_url_with_page_offsets_to_correct_page(self):
        ...
        response_page = self.client.get('/movies/recent/?page=5').json()
    ...
    def test_url_with_page_out_of_range_display_404(self):
        ...
        response = self.client.get('/movies/recent/?page=21')
...
class RecentMoviesTestCase(TestCase):
    ...
    def test_max_number_of_movies_returned_are_5(self):
        ...
        response = self.client.get('/movies/recent/')
    ...
    def test_non_get_requests_return_400(self):
        response = self.client.post('/movies/recent/')
        self.assertEqual(response.status_code, 400)

        response = self.client.put('/movies/recent/')
        self.assertEqual(response.status_code, 400)

        response = self.client.delete('/movies/recent/')
        self.assertEqual(response.status_code, 400)

        response = self.client.patch('/movies/recent/')
        self.assertEqual(response.status_code, 400)

        response = self.client.head('/movies/recent/')
        self.assertEqual(response.status_code, 400)
...
class BestMoviesTestCase(TestCase):
    ...
    def test_no_pagination(self):
        ...
        response_page1 = self.client.get('/movies/best/?page=1').json()
        response_page2 = self.client.get('/movies/best/?page=2').json()
